...
@section('admin::title','ELV Projects')
@section('admin::content')

<!-- multi file upload preview -->

<link rel="stylesheet" href="//code.jquery.com/ui/1.12.1/themes/base/jquery-ui.css">
<link rel="stylesheet" href="{{asset('css/admin/project/elv-project.css')}}">
...
.btnSubmit
{
                    ...
                    <div class="row">
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                            <div class="product-status-wrap">
                                <h4 style="color:green;">List of Projects ( ELV )</h4>
                                ...
                                <table>
                                    ...
                                    <tr><?php $i++; $project_images = json_decode($elvProject->project_images, true);  ?>
                                        <td width="3%">{{$i}}</td>
                                        <td width="7%"><img src="{{asset('assets/img/ElvProjects/'.$project_images[0])}}" alt="" /></td>
                                        <td width="10%" style="color:orange;">{{$elvProject->project_name}}</td>
                                        <td width="10%">{{$elvProject->company_name}}</td>
                                        <td width="10%" style="color:yellow;">{{$elvProject->solution_name}}</td>
                                        <td width="10%">{{$elvProject->city}}</td>
                                        <td width="10%">{{$elvProject->awarded_month}}{{$elvProject->awarded_year}}</td>
                                        <td width="30%">{{ Illuminate\Support\Str::limit($elvProject->description,350,$end='...')}}</td>
                                        <td>
                                            <button data-toggle="modal" data-target=".edit-modal-lg" data-toggle="tooltip" id="update-multi-preview-image" onclick="updateElvProject('{{$elvProject->id}}');" title="Edit" class="pd-setting-ed"><i class="fas fa-edit" aria-hidden="true"></i></button>
                                            
                                            <!-- <a href="{{url('boh/delete_elv_project',$elvProject->id)}}"> -->
                                            <button data-toggle="tooltip" title="Trash" onclick="deleteProject('{{$elvProject->id}}');" class="pd-setting-ed"><i class="fas fa-trash" aria-hidden="true"></i></button>
                                            <!-- </a> -->
                                        </td>
                                    </tr>
                                    @endforeach
         ...
         <div class="modal-dialog modal-lg"></p> 
            <div class="modal-content">
                <div class="modal-header note">
                        ...
                        <h2 class="modal-title" id="exampleModalLongTitle"> Create New ELV Project</h2>
                    
                </div>
                    <form action="{{url('boh/add_elv_project')}}" method="post"  enctype="multipart/form-data" >
                        @csrf
                                ...
                                <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                @foreach($elvSolutionNames as $elvSolutionName)
                                                <option value="{{$elvSolutionName->id}}">{{$elvSolutionName->name}}</option>
                                                @endforeach
                                               </select>
                                            </div>
                                        </div>
         ...
         <div class="modal-dialog modal-lg"></p> 
            <div class="modal-content">
                <div class="modal-header note">
                        ...
                        <h2 class="modal-title" id="exampleModalLongTitle"> Update ELV Project</h2>
                    
                </div>
                    <form action="{{url('boh/update_elv_project')}}" method="post" enctype="multipart/form-data" >
                        @csrf
                        <div class="modal-body">
    ...
    <script src="{{asset('js/admin/project/elv-project.js')}}"></script>
    <script>
 // <!-- Start Update modal -->
 ...
 var imageArray = new Array();
    ...
    function updateElvProject(id) {
        $.get( "{{url('boh/edit_elv_project')}}", { id } )
        .done(function( data ) {
            ...
            var html =  '<div class="preview-image preview-show-' + num + '">' +
                              ...
                              '<div class="image-zone"><img id="update-pro-img-' + num + '" src="../assets/img/ElvProjects/' + project_images_arr + '"></div>' +
  ...
  function updateReadImage() {
    ...
    function deleteProject(id){
        if (confirm('Are you sure want to delete this Project!')) {
            $.get( "{{url('boh/delete_elv_project')}}",{ id } )
            .done(function( data ) {
                location.reload();
