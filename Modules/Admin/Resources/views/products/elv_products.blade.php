...
@section('admin::title','ELV Products')
                    ...
                    <div class="row">
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                            <div class="product-status-wrap">
                                <h4 style="color:green;"> List of Products ( ELV )</h4>
                                ...
                                <table>
                                    ...
                                    <tr><?php $i++; $product_images = json_decode($elvProduct->product_images, true); ?>
                                        <td width="5%">{{$i}}</td>
                                        <td width="10%"><img src="{{asset('assets/img/ElvProducts/'.$product_images[0])}}" alt="" /></td>
                                        <td width="10%">{{$elvProduct->solution_name}} 
                                        </td>
                                        <td width="10%" style="color:orange;">{{$elvProduct->product_name}}</td>
                                        <td width="10%">{{$elvProduct->brand_name}}</td>
                                        <td width="10%">{{$elvProduct->price}}</td>
                                        <?php 
                                        ...
                                        <td>
                                            <button data-toggle="modal" data-target=".edit-modal-lg" data-toggle="tooltip" onclick="updateElvProduct('{{$elvProduct->id}}');"  id="category" title="Edit" class="pd-setting-ed multi-preview"><i class="fas fa-edit" aria-hidden="true"></i></button>
                                            
                                            
                                            <button data-toggle="tooltip" title="Trash" class="pd-setting-ed" onclick="deleteProduct('{{$elvProduct->id}}');" ><i class="fas fa-trash" aria-hidden="true"></i></button>
                                        </td>
                                    </tr>
                                    @endforeach
                                    @endif
         ...
         <div class="modal-dialog modal-lg"></p> 
            <div class="modal-content">
                <div class="modal-header note">
                        ...
                        <h2 class="modal-title" id="exampleModalLongTitle"> Create New Product ( ELV )</h2>
                    
                </div>
                    <form action="{{url('boh/add_elv_product')}}" method="post"  enctype="multipart/form-data" >
                        @csrf
                        <div class="modal-body">
                                <div class="row">
                                        <div class="col-md-6">
                                                ...
                                                @if(isset($elvSolutionNames))
                                                @foreach($elvSolutionNames as $elvSolutionName)
                                                <option value="{{$elvSolutionName->name}}">{{$elvSolutionName->name}}</option>
                                                @endforeach
                                                @endif
                                               </select>
                                                </div>
         ...
         <div class="modal-dialog modal-lg"></p>
            <div class="modal-content">
                <div class="modal-header note">
                        ...
                        <h2 class="modal-title" id="exampleModalLongTitle"> Update Product ( ELV )</h2>
               </div>
                    <form action="{{url('boh/update_elv_product')}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="modal-body">
...
var imageArray = new Array();
    ...
    function updateElvProduct(id) {
        $.get( "{{url('boh/edit_elv_product')}}", { id } )
        .done(function( data ) {
        ...
        $.each(specification_array, function (index ,specification_arr) {
            ...
            var html =  '<div class="preview-image preview-show-' + num + '">' +
                              ...
                              '<div class="image-zone"><img id="update-pro-img-' + num + '" src="../assets/img/ElvProducts/' + product_images_arr + '"></div>' +
  ...
  function updateReadImage() {
    ...
    function deleteProduct(id){
        if (confirm('Are you sure want to delete this Product!')) {
            $.get( "{{url('boh/delete_elv_product')}}",{ id } )
            .done(function( data ) {
                location.reload();
